...
public class TiffFilePreviewImpl implements FilePreview {
    ...
    public TiffFilePreviewImpl(PictureFilePreviewImpl pictureFilePreview, OtherFilePreviewImpl otherFilePreview) {
    ...
    public String filePreviewHandle(String url, Model model, FileAttribute fileAttribute) {
        ...
        if ("tif".equalsIgnoreCase(tifPreviewType)) {
            ...
            String inputFileName = WebUtils.getFileNameFromURL(url);
            String inputFileNamePrefix = inputFileName.substring(0, inputFileName.lastIndexOf("."));

            String strLocalTif = KkFileUtils.getDateDir(fileDir, inputFileName);
            ...
            if ("pdf".equalsIgnoreCase(tifPreviewType)) {
                // 以PDF模式预览的过程
                String filePdfPath = KkFileUtils.getDateDir(fileDir, inputFileNamePrefix + ".pdf");
                File filePdf = new File(filePdfPath);
                // 如果本地不存在对应的pdf，则调用转换过程。否则直接用现有的pdf文件
                if (!filePdf.exists()) {
                    filePdf = ConvertPicUtil.convertTif2Pdf(strLocalTif, filePdfPath);
                }

                // 如果pdf已经存在，则将url路径加入到对象中，返回给页面
                if (filePdf != null && filePdf.exists()) {
                    String pdfUrl = baseUrl + KkFileUtils.getUrlRelativePath(filePdfPath);
                ...
                // 以JPG模式预览的过程
                String strJpgFilePathName = KkFileUtils.getDateDir(fileDir, inputFileNamePrefix + ".jpg");
                ...
                for (String strJpg : listPic2Jpg) {
                    listImageUrls.add(baseUrl + StrUtil.replace(strJpg, "\\", "/"));
                }
